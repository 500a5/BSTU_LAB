...
/*
    //ui->fulNameEdit->clear();
    //ui->nameEdit->clear();
    //ui->handleEdit->clear();
    //ui->nameEdit->clear();
  //  ui->fullNameEdit->clear();
  //  ui->handleEdit->clear();
    QString fullname, name, handle;
    fullname = ui->fullNameEdit->text();
    name = ui->nameEdit->text();
    handle = ui->handleEdit->text();

    //qDebug() << name << fullname << handle;
    HMODULE h;

    if (name != ""){
        //вывод дескрпитора и полного имени
        getInfoByName(h, name, fullname);
        ui->fullNameEdit->setText(fullname);
        ui->nameEdit->setText(name);
        ui->handleEdit->setText(QString("%1").arg((int)h));
        //qDebug() << name << fullname << h;
        return;
    }


    if (handle != ""){
        //вывод информации о имени и полном имени
        h = (HMODULE)handle.toInt();
        getInfoByHandle(h, name, fullname);
        showInfo(h, name, fullname);
        qDebug() << name << fullname << h;
        return;
    }

    if (fullname != ""){
        //вывод дескриптора и имени
        getInfoByFullName(h, name, fullname);
        showInfo(h, name, fullname);
        qDebug() << name << fullname << h;
        return;
    }
