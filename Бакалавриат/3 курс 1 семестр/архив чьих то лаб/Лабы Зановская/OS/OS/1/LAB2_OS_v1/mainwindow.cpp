...
{
    ui->setupUi(this);
    connect(ui->handle_LineEdit, SIGNAL(textEdited(QString)), this, SLOT(handle_LineEdit_changed()));
    connect(ui->name_LineEdit, SIGNAL(textEdited(QString)), this, SLOT(name_LineEdit_changed()));
    connect(ui->fname_LineEdit, SIGNAL(textEdited(QString)), this, SLOT(fname_LineEdit_changed()));
    ui->processes_Table->setColumnCount(5);
    ui->processes_Table->setItemDelegateForColumn(0, new NonEditTableColumnDelegate);
    ui->processes_Table->setItemDelegateForColumn(1, new NonEditTableColumnDelegate);
    ui->processes_Table->setItemDelegateForColumn(2, new NonEditTableColumnDelegate);
    ui->processes_Table->setItemDelegateForColumn(3, new NonEditTableColumnDelegate);
    ui->processes_Table->setItemDelegateForColumn(4, new NonEditTableColumnDelegate);
    ui->modules_Table->setColumnCount(4);
    ui->modules_Table->setItemDelegateForColumn(0, new NonEditTableColumnDelegate);
    ui->modules_Table->setItemDelegateForColumn(1, new NonEditTableColumnDelegate);
    ui->modules_Table->setItemDelegateForColumn(2, new NonEditTableColumnDelegate);
    ui->modules_Table->setItemDelegateForColumn(3, new NonEditTableColumnDelegate);
    ui->threads_Table->setColumnCount(2);
    ui->threads_Table->setItemDelegateForColumn(0, new NonEditTableColumnDelegate);
    ui->threads_Table->setItemDelegateForColumn(1, new NonEditTableColumnDelegate);
...
void MainWindow::handle_LineEdit_changed()
{
    ui->name_LineEdit->setReadOnly(true);
    ui->fname_LineEdit->setReadOnly(true);
}

void MainWindow::name_LineEdit_changed()
{
    ui->handle_LineEdit->setReadOnly(true);
    ui->fname_LineEdit->setReadOnly(true);
}

void MainWindow::fname_LineEdit_changed()
{
    ui->name_LineEdit->setReadOnly(true);
    ui->handle_LineEdit->setReadOnly(true);
}
...
{
    QString name, fname;
    // 1 вариант работы функции
    if (ui->handle_LineEdit->text() == "" &&
        ui->name_LineEdit->text() == "" &&
        ui->fname_LineEdit->text() == "")
    {
        // Получаем имя и полное имя
        getModuleFileName(NULL, fname, name);
        ui->fname_LineEdit->setText(fname);
        ui->name_LineEdit->setText(name);
        // Получаем дескриптор модуля
        ui->handle_LineEdit->setText(QString("0x%1").arg((int)GetModuleHandle(NULL), 0, 16));
    }
    else if (ui->handle_LineEdit->isReadOnly() == false) {
        if (getInfoByHandle((HMODULE)ui->handle_LineEdit->text().toInt(0, 0), fname, name)) {
           ui->fname_LineEdit->setText(fname);
           ui->name_LineEdit->setText(name);
        }
        else {
    ...
    else if (ui->name_LineEdit->isReadOnly() == false) {
        HMODULE hModule;
        if (getInfoByName(hModule, fname, ui->name_LineEdit->text())) {
            ui->handle_LineEdit->setText(QString("0x%1").arg((int)hModule, 0, 16));
            ui->fname_LineEdit->setText(fname);
        }
        else {
            QMessageBox::warning(this, "Ошибка!", "Некорректное имя модуля.");
            on_clear_Button_clicked();
        }
    }
    else if (ui->fname_LineEdit->isReadOnly() == false) {
        HMODULE hModule;
        if (getInfoByFName(hModule, ui->fname_LineEdit->text(), name)) {
            ui->handle_LineEdit->setText(QString("0x%1").arg((int)hModule, 0, 16));
            ui->name_LineEdit->setText(name);
        }
        else {
            QMessageBox::warning(this, "Ошибка!", "Некорректное полное имя модуля.");
            on_clear_Button_clicked();
        }
    }
    else {
        QMessageBox msgBox;
        msgBox.setText("Undefined situation.");
        msgBox.exec();
        on_clear_Button_clicked();
    }
    ui->handle_LineEdit->setReadOnly(false);
...
void MainWindow::on_clear_Button_clicked()
{
    ...
    ui->name_LineEdit->setReadOnly(false);
    ui->fname_LineEdit->setReadOnly(false);
    ui->handle_LineEdit->setText("");
    ui->name_LineEdit->setText("");
    ui->fname_LineEdit->setText("");
}
