...
// Каркасное приложение для демонстрации буферизованного вывода функцией glDrawArrays
...
void MainGLWidget::glCube()
{
    ...
    GLubyte C[] = {255, 120,  39,   // Цвет первой грани
                    50, 106, 200,   // Цвет второй грани
                    60, 140,  10,   // ...
                    50, 180, 150,
                   250,  80, 150,
                   100,  55, 180};  // Цвет шестой грани
    ...
    // Параметры 24 вершин  6 граней (цвет, ккординаты)
    // 6 граней, каждая состоит из 4 вершин
    float vertices[24][3];
    GLubyte colors[24][3];

    // Готовим данные для функции glDrawArrays.
    // Копируем координаты вершин в массив vertices.
    // В массивах vertices и colors информация хранится по граням.
    // Первые 4 элемента (0-3) - первая грань, 4-7 - вторая и т.д.
    for (int i=0; i < 24; i++)
    {
        memcpy(&vertices[i], &V[indeces[i]], 3 * sizeof(float));
        memcpy(&colors[i]  , &C[i/4], 3 * sizeof(GLubyte));
    }
