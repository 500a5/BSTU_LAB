    ...
    while (!exit_f) {
        ...
        while (it + 4 <= raw_str.end()) {
           ...
           // std::cout << y.str_epsilon() << " = ";
            ...
            std::cout << "\tРез. декодирования: i(x)=" << y.str() << std::endl;// << "=" << y.str_bit() << std::endl;
            ...
            if ((raw_str.length() - count * 4) < 4) {
                ...
                std::cout << "Содержимое файла успешно закодировано." << std::endl;
                exit(0);
            }
        }
        std::cout << "Содержимое файла успешно закодировано." << std::endl;
        exit_f = (choose("Повторить операцию кодирования/декодирования? 1 - да, 2 - нет", 1, 2) == 2);
    }
    return 0;
}
