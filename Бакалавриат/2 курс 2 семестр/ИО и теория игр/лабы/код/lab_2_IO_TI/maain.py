...
def getFirstTable(matr, z):
    table = [[0 for y in range(len(matr[0]) + 1)] for x in range(len(getBasicList(matr)[0]) + 2)]
    table[0][0] = "Базовые переменные"
    table[0][1] = "Свободные переменные"
    for i in range(2, len(table[0])):
        table[0][i] = 'X' + str(i - 1)
    basic, stringBasic = getBasicList(matr)
